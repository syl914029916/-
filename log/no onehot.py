...
# 需要进行数据无量纲化处理的列
standard_scaler_list = ['month', 'job', 'education', 'contact', 'day', 'marital', 'age', 'balance', 'duration', 'campaign', 'pdays', 'previous', 'poutcome']
# 需要转换为0-1二值编码的列
...
for i in C:
    lr = LR(penalty='l2', solver='liblinear', C=i, random_state=420)
...
plt.show()
# 交叉验证超参C最优解：
# 0.9100022697386072 0.11

# 训练数据
lr = LR(penalty='l2', solver='liblinear', C=0.01, random_state=420)
...
'''
print("测试集精确度和召回率")
print(precision_score(Ytest, get_ypred(Xtest, lr, threshold), labels=[1, 0]))
print(recall_score(Ytest, get_ypred(Xtest, lr, threshold), labels=[1, 0]))
...
result2.to_csv(r'C:\Users\902sx\Desktop\graduate\result_lr1.csv', index=False)
